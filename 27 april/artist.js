...
router.get('/statusbasecategoryname/:status/:baseprice/:category/:name', (req, res) => {
  ...
  //   // Search by name + baseprice
  // router.get('/namebaseprice/:name/:baseprice', (req, res) => {
  ...
  //                 a.ArtistName LIKE ?
  //                 AND a.BasePrice >= ?
  //             ORDER BY a.BasePrice ASC
  //         `;

  //         connection.query(query, [`%${artist_name}%`, artist_baseprice], function (error, results) {
  //           if (error) {
  //               return res.status(500).send({ error: true, message: 'Database error' });
  //           }
  ...
  //       });
  // //   });
  //   // search by category + baseprice
  //   router.get('/categorybaseprice/:category/:baseprice', (req, res) => {
  //     let artist_category = req.params.category;
  //     if (!artist_category) {
  ...
  //           connection.query(query, [`%${artist_category}%`, artist_baseprice], function (error, results) {
  //             if (error) {
  //                 return res.status(500).send({ error: true, message: 'Database error' });
  //             }
